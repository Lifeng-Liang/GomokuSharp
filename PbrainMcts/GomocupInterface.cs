...
namespace PbrainMcts
{
    public abstract class GomocupInterface
    {
        ...
        /* create the board and call Response("OK"); or Response("ERROR Maximal board size is .."); */
        abstract public void brain_restart(); /* delete old board, create new board, call Response("OK"); */
        ...
        abstract public void brain_block(int x, int y);
        /* square [x,y] belongs to a winning line (when info_continuous is 1) */
        abstract public int brain_takeback(int x, int y);
        ...
        private void do_command()
        {
            ...
            switch (get_cmd_param(cmd, out param))
            {
                ...
                case "start":
                    ...
                    {
                        width = 0;
                        Response("ERROR bad START parameter");
                    }
                    else
                    {
                        height = width;
                ...
                case "rectstart":
                    ...
                    {
                        width = height = 0;
                        Response("ERROR bad RECTSTART parameters");
                    }
                    else
                    {
                        start();
                    ...
                    for (;;) /* fill the whole board */
                    {
                        ...
                        else
                        {
                            if (!cmd.Equals("done", StringComparison.InvariantCultureIgnoreCase))
                                Response("ERROR x,y,who or DONE expected after BOARD");
                            break;
                        }
                    }
                    turn();
